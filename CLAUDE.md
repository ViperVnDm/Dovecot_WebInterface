...
**Multi-select.** Each row has a `name="ids"` checkbox (Alpine `x-model="sel"` on the partial root). "Approve selected" / "Reject selected" post the checked ids via `hx-include` to `POST /api/agent/suggestions/{approve,reject}-selected` (form field `ids`, repeated; empty → 400). Approve-selected is best-effort like approve-all-bans — a target that fails stays pending and is named in the `notice` banner the re-render shows — and applies `allowlist` actions *before* `ban` actions so a batch that allowlists and bans the same target refuses the ban. Single and bulk approve share `_apply_suggestion()`, which mutates the allowlist in memory; the caller persists it once via `_save_allowlist()`. Ids already reviewed are skipped and counted, not errors. The 30s poll carries a trigger filter that skips the refresh while any `.suggestion-select` is checked, so a refresh can't wipe a selection in progress.
